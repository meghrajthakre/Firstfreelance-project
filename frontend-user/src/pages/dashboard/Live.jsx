...
import { GiSoccerBall, GiTennisBall, GiPerson, GiVote, GiTrophyCup } from "react-icons/gi";
import { CricketIcon } from "@phosphor-icons/react";

const SPORTS = [
  { id: "cricket",    label: "CRICKET",    icon: <CricketIcon  size={20} /> },
  { id: "football",   label: "FOOTBALL",   icon: <GiSoccerBall size={20} /> },
  { id: "tennis",     label: "TENNIS",     icon: <GiTennisBall size={20} /> },
  { id: "kabaddi",    label: "KABADDI",    icon: <GiPerson     size={20} /> },
  { id: "elections",  label: "ELECTIONS",  icon: <GiVote       size={20} /> },
  { id: "tournament", label: "TOURNAMENT", icon: <GiTrophyCup  size={20} /> },
];
          ...
          {SPORTS.map((sport) => {
            ...
            return (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-lg font-rajdhani font-semibold text-sm tracking-wide transition-all duration-200 cursor-pointer"
                style={
                  isActive
                    ? {
      ...
      {matches.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {matches.map((match) => (
            <div
              key={match.id}
              className="rounded-xl overflow-hidden cursor-pointer transition-transform duration-150 hover:-translate-y-0.5"
              style={{
                backgroundColor: "var(--color-input-bg)",
                border: "1px solid var(--color-border)",
                boxShadow: "0 2px 8px rgba(0,0,0,0.07)",
                display: "flex",
                flexDirection: "row",   /* always row — no column on mobile */
                minHeight: "80px",
              }}
            >
              {/* Sport icon panel — slim on mobile, wider on sm+ */}
              <div
              ...
              >
                <span style={{ color: "var(--color-text-muted)", fontSize: "22px", display: "flex" }}>
                  {activeSport?.icon}
                </span>
              </div>

              {/* Match info — grows to fill space */}
              <div
                className="flex flex-col justify-center gap-1"
                style={{ flex: 1, padding: "10px 12px", minWidth: 0 }}
              >
                <h3
                  className="font-rajdhani font-bold tracking-wide leading-tight"
                  style={{
                    color: "var(--color-primary)",
                ...
                <div className="flex gap-3 flex-wrap" style={{ marginTop: "2px" }}>
                  <span
                    className="font-nunito"
                    style={{ color: "var(--color-text-dark)", opacity: 0.6, fontSize: "11px" }}
                  >
                    Match Bets : <strong style={{ color: "var(--color-primary)" }}>{match.matchBets}</strong>
                  </span>
                  <span
                    className="font-nunito"
                    style={{ color: "var(--color-text-dark)", opacity: 0.6, fontSize: "11px" }}
                  >
                    Session Bets : <strong style={{ color: "var(--color-primary)" }}>{match.sessionBets}</strong>
                  </span>
                </div>
              </div>

              {/* Date box — always on the right */}
              <div
              ...
              >
                <span
                  ...
                  style={{ color: "var(--color-input-bg)", fontSize: "26px" }}
                >
                  {match.day}
                </span>
                <span
                  className="font-rajdhani font-semibold"
                  style={{ color: "var(--color-text-muted)", fontSize: "11px", marginTop: "1px" }}
                >
                  {match.month}
                </span>
                <span
                  className="font-nunito"
                  style={{ color: "var(--color-text-muted)", fontSize: "10px", marginTop: "3px", whiteSpace: "nowrap" }}
                >
                  {match.time}
                </span>
              </div>
