 ...
 *
 * Market bets:
 *   BACK (Lagai): profit = (odds - 1) * stake
 *   LAY  (Khai):  profit = stake
 *
 * Session bets (Yes / No):
 *   profit = (stake * rate) / 100
 *   rate is stored in the odds field for sessions
 */
const calculateProfit = (type, odds, stake) => {
  const o = parseFloat(odds) || 0;
  const s = parseFloat(stake) || 0;
  if (type === "Lagai") return parseFloat(((o - 1) * s).toFixed(2));
  if (type === "Khai")  return parseFloat(s.toFixed(2));
  // Session Yes / No
  return parseFloat(((o / 100) * s).toFixed(2));
};
 ...
 *
 * Market bets:
 *   BACK (Lagai): liability = stake           (lose your stake if wrong)
 *   LAY  (Khai):  liability = (odds - 1) * stake  (pay out if runner wins)
 *
 * Session bets:
 *   liability = stake  (you lose your stake if the outcome goes against you)
 */
const calculateLiability = (type, odds, stake) => {
  const o = parseFloat(odds) || 0;
  const s = parseFloat(stake) || 0;
  if (type === "Lagai") return parseFloat(s.toFixed(2));
  if (type === "Khai")  return parseFloat(((o - 1) * s).toFixed(2));
  return parseFloat(s.toFixed(2));
};

/**
 * calculateExposure
 * Sum of liabilities across all OPEN / MATCHED bets.
 */
...
/**
 * calcRunnerPnl
 * Potential PnL for a market runner across all active bets.
 * Positive = you profit if runner wins, negative = you lose.
 */
const calcRunnerPnl = (bets, runner) => {
  ...
  bets
    ...
    .forEach(b => {
      if (b.type === "Lagai") pnl += b.profit;    // BACK: runner wins → you profit
      if (b.type === "Khai")  pnl -= b.liability;  // LAY:  runner wins → you pay
    });
  return parseFloat(pnl.toFixed(2));
};

/* ─── primitive UI atoms (unchanged from original) ─── */
const card = {
  backgroundColor: C.inputBg, borderRadius: 14,
  border: `1.5px solid ${C.border}`, overflow: "hidden",
};
const sectionBar = (extra = {}) => ({
  backgroundColor: C.sectionHdr, padding: "9px 14px",
...
// TD gains a `red` prop for negative PnL colouring
...
const MatchDetail = () => {
  ...
  const [betSlip,     setBetSlip]     = useState(null);   // { runner, type, odds, isSession }
  const [stake,       setStake]       = useState("500");
  const [confirmed,   setConfirmed]   = useState(false);
  const [chatOpen,    setChatOpen]    = useState(false);
  const [bets,        setBets]        = useState([]);     // all placed bets
  const [myBetsOpen,  setMyBetsOpen]  = useState(false);  // My Bets panel toggle

  /* live odds jitter — fixed: ref prevents sessions in dep-array */
  ...
  /* ── openBet: tag isSession so logic helpers pick correct formula ── */
  ...
  /* ── Derived slip values (recalculate on every render) ── */
  ...
  const handlePlaceBet = () => {
    ...
    const newBet = {
      ...
      status:    "MATCHED",           // instantly matched simulation
  ...
  const sessionPnl = (name) =>
    bets
      ...
      .reduce((sum, b) => sum + (b.type === "Yes" ? b.profit : -b.liability), 0);
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: C.bgMain, fontFamily: "var(--font-nunito)", paddingBottom: 90 }}>
      <div style={{ maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column", gap: 10, padding: "12px 10px" }}>
        ...
        {/* ══ EXPOSURE METER (only when bets exist) ══ */}
        {totalExposure > 0 && (
          <div style={{
            background: C.primary, borderRadius: 10, padding: "8px 14px",
            display: "flex", justifyContent: "space-between", alignItems: "center",
            border: `1px solid ${C.banner}`,
          }}>
            <span style={{ fontSize: 11, color: "rgba(255,255,255,0.55)", fontFamily: "var(--font-rajdhani)", letterSpacing: 0.4 }}>
              TOTAL EXPOSURE
            </span>
            <span style={{
              fontSize: 13, fontWeight: 700, fontFamily: "var(--font-rajdhani)",
              ...
              {marketRows.map((row, i) => {
                ...
                return (
                  <tr key={i} style={{ backgroundColor: row.hl ? C.hlRow : "transparent" }}>
                    ...
                    {/* LAGAI = BACK */}
                    <TD>
                      <OddsBtn color={C.lagai} label={row.lagaiO}
                        onClick={() => openBet(row.full, "Lagai", row.lagaiO, false)} />
                    </TD>
                    {/* KHAI = LAY */}
                    <TD>
                      <OddsBtn color={C.khai} label={row.khaiO}
                        onClick={() => openBet(row.full, "Khai", row.khaiO, false)} />
                    </TD>
                    {/* Dynamic PnL per runner */}
              ...
              {sessions.map((s, idx) => {
                ...
                return (
                  <tr key={idx} style={{ backgroundColor: idx % 2 === 0 ? "#f4f8fd" : C.inputBg }}>
                    ...
                    {/* Dynamic session PnL */}
                    <TD green={pnl > 0} red={pnl < 0}>
                      {pnl !== 0 ? (pnl > 0 ? `+${pnl.toFixed(2)}` : pnl.toFixed(2)) : 0}
                    </TD>
                  </tr>
                );
              })}
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              <tr>
                ...
                <TD><OddsBtn color={C.lagai} label="90"  onClick={() => openBet("Match Tie", "Lagai", "90",  false)} /></TD>
                <TD><OddsBtn color={C.khai}  label="100" onClick={() => openBet("Match Tie", "Khai",  "100", false)} /></TD>
              </tr>
            </tbody>
          </table>
        </div>
                    ...
                    <tr key={b.id} style={{ backgroundColor: C.inputBg }}>
                      ...
                      <TD>
                        <span style={{
                          ...
                          color: "#fff", fontFamily: "var(--font-rajdhani)",
        ...
        <div style={{
          ...
          {!confirmed ? (
            <>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 13, fontWeight: 700, color: "#fff", fontFamily: "var(--font-rajdhani)", letterSpacing: 0.4 }}>
                    {/* Show BACK / LAY label for market bets */}
                  ...
                  {betSlip.isSession && (
                    <span style={{
                      ...
                      fontSize: 10, padding: "2px 8px", borderRadius: 4, fontFamily: "var(--font-rajdhani)",
                    }}>SESSION</span>
                  )}
                </div>
              ...
              {overLimit && (
                <div style={{
                  background: "rgba(224,85,96,0.18)", border: "1px solid #e05560",
              ...
              {/* Stats — correctly computed profit & liability */}
              ...
              <button
                ...
                style={{
                  width: "100%", background: overLimit ? "#4a4a5a" : C.yesBg,
